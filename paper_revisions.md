...
## 14. Alert Rate-Limiting Mechanisms (Vector-Specific)

**Implementation Detail:**
NoEsc currently uses two different anti-flood strategies depending on vector:

- **Vector 1 (SUID abuse)** and **Vector 3 (sensitive tampering)**:
  Per-user/per-key cooldown via `ALERT_COOLDOWN_SECS = 10`.
- **Vector 2 (SudoMisuse)**:
  Burst aggregation and summary notifications (not the same 10-second cooldown path).
...
- Subsequent detections for Vector 1/3 (within 10 seconds): Suppressed
...
## 15. Exec-Only Scoring Gate for SudoMisuse

**Original Proposal (Section H):**
High-risk sudo actions were described conceptually, but did not explicitly separate command launch events from internal permission syscalls.

**Implemented Logic:**
SudoMisuse scoring for dangerous commands is applied only to **exec-like launch events** (e.g., `benign_exec`/execve), not to every privileged syscall generated by the same process.

**Reason for Change:**
Without this gate, internal follow-up syscalls (e.g., chmod/chown/cp permission operations) can over-inflate score and create operational noise during legitimate maintenance workflows (package updates, kernel installs).

**Impact:**
Improves signal quality by scoring intent (command launch) rather than low-level syscall volume.

## 16. SudoMisuse Burst Episode Aggregation (Mega-Summary Mode)

**Original Behavior in Earlier Iteration:**
Window rollover could produce repeated summary notifications within one sustained high-frequency stream.

**Current Implemented Behavior:**
- First CRITICAL in an episode: immediate notification.
- Subsequent CRITICALs in the same episode: suppressed and counted.
- Episode end: one summary notification emitted when a new CRITICAL arrives after a quiet gap (`>= SUDO_NOTIFICATION_BURST_WINDOW_SECS`).
- Stream end/shutdown: pending suppressed count is emitted immediately by flush.

**Why This Matters:**
This reduces intrusive desktop spam while preserving forensic visibility in logs and still providing user-facing awareness of sustained suspicious bursts.

## 17. Maintenance Mode TTL for Notification Suppression

**Implemented Logic:**
NoEsc supports operator-controlled maintenance mode using a TTL file:
`/etc/noesc/sudo_maintenance_mode.until`.

**Behavior:**
- During active maintenance TTL, SudoMisuse desktop notifications are suppressed.
- Cached checks reduce file-read overhead (`MAINTENANCE_CACHE_REFRESH_SECS`).

**Rationale:**
Planned maintenance windows (updates, reconfiguration, package operations) can generate expected privileged activity; temporary suppression prevents alert fatigue while preserving core detection and log outputs.

## 18. Realtime Replay Tooling for Thesis Validation

**Implemented Assets:**
- `sample_set/yay_kernel_install_extreme_10s.log`
- `sample_set/yay_kernel_install_extreme_10s_dense.log`
- `scripts/notification/replay_extreme_10s_realtime.sh`

**Purpose:**
Enable reproducible, wall-clock demonstrations of high-frequency SudoMisuse streams in approximately 10 seconds for evaluation, screenshots, and defense demonstrations.

**Method:**
Replay script preserves inter-event timing deltas from audit timestamps and pipes to the daemon, approximating real-time event flow.

## 19. Syslog vs Auditd Architectural Clarification
...
## 20. Rule Externalization Scope Adjustment

**Original Proposal (Architecture):**
Detection rules were described as externally configurable (e.g., JSON/YAML) and loaded at startup to avoid recompilation.

**Implemented Reality:**
Core detection logic is implemented in C++ code paths, while selected operational controls are externalized (e.g., SUID whitelist file and maintenance-mode TTL file).

**Reason for Change:**
Keeping core vector logic in code improved implementation speed, predictability, and debug visibility during thesis experimentation. The trade-off is lower runtime rule flexibility than originally proposed.
...
- Vector-specific anti-flood controls (Revision 14)
...
- Maintenance mode TTL suppression (Revision 17)
- Realtime replay tooling for validation (Revision 18)
- Syslog vs auditd architecture clarification (Revision 19)

### Academic Environment Adaptations:
- Reduced false positives for student coursework (Revision 9)
- Alert fatigue mitigation (Revisions 9, 10, 12, 16, 17)
