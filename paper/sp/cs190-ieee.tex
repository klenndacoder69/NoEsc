...
The performance overhead of each detection engine will be quantified using both a macro and a micro benchmark. For each test, the average execution time across five runs will be measured under three distinct conditions:
\begin{enumerate}
\item \textbf{Baseline:} Execution with no detector running.
\item \textbf{Rule-Based Daemon:} Execution with the heuristic rule-based daemon active.
\item \textbf{ML-Based Daemon:} Execution with the machine learning-based daemon active.
\end{enumerate}
The overhead will be calculated as the percentage increase in execution time relative to the baseline.
\subsubsection{Macro Benchmark}
The macro benchmark measures the daemon's impact on real-world performance by executing a CPU- and I/O-heavy task: compiling a large C++ project (e.g., the LLVM compiler).
\subsubsection{Micro Benchmark}
The micro benchmark measures raw event processing throughput by executing a simple system call, such as \texttt{getpid()}, in a tight loop. This stress test, a common practice in evaluating host-based security tools \cite{qiang2018}, evaluates the daemon's performance under a high rate of event generation.
